...
- **SHOULD** 在 `it` 中体现 Arrange（准备）、Act（执行）、Assert（断言）三个阶段，可通过空行或注释表达，不强制逐字注释。
- 对非平凡测试（多步交互、异步流程、多个前置条件），**MUST** 明确阶段边界，避免把多步操作与断言混在一行。
- 对“纯渲染即断言”或“单次调用即断言”的极简场景，允许省略显式 `Act` 注释。

### 4. 用例绝对隔离

- **MUST** 重置会跨用例共享的 mocks、timers、全局对象与模块缓存，确保用例互不污染。
- **SHOULD** 优先在 `beforeEach()` / `afterEach()` 统一清理；若仅在单个用例内创建临时桩，可在该用例内就地恢复。
...
- 对同构断言的重复样例（通常 >= 3 组输入/输出），**SHOULD** 使用 `it.each()` 参数化测试；少量样例可保留显式 `it` 以提升可读性。
...
- 测试名称可直接读出“场景 + 期望”。
- 异步行为均已正确等待，不依赖竞态时序。
- 对 DOM 组件测试优先语义化查询（如 `getByRole`），减少脆弱选择器；Hook/纯函数测试以公共返回值和可观察副作用断言为主。
- 关键边界与错误路径具备覆盖。
- 无重复样板和无效断言。

## 参考模板（按项目栈与场景调整，不要求逐字照搬）
